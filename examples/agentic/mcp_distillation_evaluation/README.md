...
- **[uv](https://docs.astral.sh/uv/)** — sdg_hub uses `uv` for package management.
  The startup scripts install dependencies via `uv pip install`; `start_servers.sh`
  also installs shared `fastmcp` for servers that depend on the standalone package.
...
bash start_servers.sh  # installs shared fastmcp + per-server deps; warns on install failures
